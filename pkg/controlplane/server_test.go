...
func newTestServer(t *testing.T, store StateStore) *Server {
	t.Helper()
	server, err := New(context.Background(), testConfig(), store, strings.Repeat("a", 64), BuildInfo{GoVersion: "test-runtime"})
	if err != nil {
		t.Fatal(err)
	}
	return server
...
func TestBootstrapSurvivesRetryAndLostCommitWithoutAdoptingAnotherInstallation(t *testing.T) {
	...
	for range 8 {
		...
		go func() {
			defer wait.Done()
			if _, err := New(context.Background(), testConfig(), store, first.operatorToken, BuildInfo{}); err != nil {
				t.Error(err)
			}
		}()
	}
